...
export interface ExpenseRegisterFormData {
  history?: BankAccountHistory;
}

export interface ExpenseRegisterFormResult {
...
@Component({
  selector: 'app-expense-register-form-modal',
  imports: [
    ReactiveFormsModule,
    DialogFrame,
    MatFormFieldModule,
  ...
  templateUrl: './expense-register-form-modal.html',
  styleUrl: './expense-register-form-modal.scss',
})
export class ExpenseRegisterFormModal implements OnInit {
  private fb = inject(FormBuilder);
  private dialogRef = inject(MatDialogRef<ExpenseRegisterFormModal>);
  private bankAccountService = inject(SPBankAccount);
  private expenseService = inject(SPBankExpense);
  private data: ExpenseRegisterFormData = inject(MAT_DIALOG_DATA);
  ...
  readonly transactionTypes = toSignal(this.expenseService.expenseTransactionTypes(), { initialValue: [] });
  ...
  onSave(): void {
    ...
    } satisfies ExpenseRegisterFormResult);
  }
