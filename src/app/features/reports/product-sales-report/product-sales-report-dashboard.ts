...
export class ProductSalesReportDashboard implements OnInit {
  private serviceOrderProvider = inject(SPServiceOrder);
  private snackBar = inject(MatSnackBar);

  readonly rows = signal<ProductSalesReportRow[]>([]);
  readonly loading = signal(false);

  readonly columns = ['rank', 'product_name', 'quantity', 'income'];
