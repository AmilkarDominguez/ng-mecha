...
export class SPBankTransactionType {
  private supabase: SupabaseClient;
  private data$ = new BehaviorSubject<BankTransactionType[]>([]);
  private listening = false;

  private readonly TABLE_NAME: string = 'bank_transaction_types';

  constructor() {
    this.supabase = createClient(environment.supabaseUrl, environment.supabaseKey);
  }

  public get(): Observable<BankTransactionType[]> {
  ...
  public add(item: BankTransactionType): Observable<BankTransactionType[]> {
  ...
  public update(item: BankTransactionType): Observable<BankTransactionType[]> {
  ...
  public delete(id: string): Observable<BankTransactionType[]> {
    return from(
  ...
  public listen(): Observable<BankTransactionType[]> {
    this.get().subscribe((items) => this.data$.next(items));

    if (!this.listening) {
      this.listening = true;
      this.supabase
        .channel('bank_transaction_types-changes')
